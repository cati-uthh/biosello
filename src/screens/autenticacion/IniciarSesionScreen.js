    ...
    const ejecutarPeticionLogin = async (identificadorVal, contrasenaVal, esPrimerIngreso = true) => {
        ...
        try {
            ...
            if (!response.ok || result.success === false) {
                ...
                let mensaje = result.error || 'Credenciales incorrectas.';
                if (typeof mensaje === 'string' && mensaje.toLowerCase().includes('verificar')) {
                    mensaje = 'Tu cuenta aún no está verificada o las credenciales no coinciden.';
                }
            ...
            const datosSesion = {
                ...(result.usuario || result.data || result.user || {}),
                ...result,
                token: result.token || result.accessToken || result.data?.token || result.usuario?.token || null
            };
            ...
            if (esPrimerIngreso) {
                ...
                if (!cuentaGuardada || cuentaGuardada.identificador !== idLimpio) {
                    preguntarActivacionBiometria(idLimpio, contrasenaVal, datosSesion);
                } else {
                    // Actualizar contraseña en caso de cambio
                    await guardarBiometriaCuenta(idLimpio, contrasenaVal);
                    finalizarLogin(datosSesion);
                }
            } else {
                finalizarLogin(datosSesion);
            }

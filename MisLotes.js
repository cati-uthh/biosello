...
const FILTROS_RAPIDOS = [
    ...
    { label: 'Activos', value: 'activo', estado: 'activo' },
    { label: 'Procesados', value: 'procesado', estado: 'procesado' },
    { label: 'Vendidos', value: 'vendido', estado: 'vendido' },
    { label: 'Caducados', value: 'caducado', estado: 'caducado' },
    { label: 'Fecha de ingreso', value: 'fecha', tipo: 'fecha' }
];
...
const colorEstado = (estado) => {
    ...
    if (estado === 'procesado') return { fondo: '#eff6ff', texto: '#1d4ed8' };
    if (estado === 'caducado') return { fondo: '#fef2f2', texto: '#b91c1c' };
    return { fondo: '#f8fafc', texto: '#334155' };
};
...
const crearFormularioEdicion = (lote) => ({
    ...
    animal: {
        ...
        clasificacion: lote?.clasificacion || 'VAQUILLA',
...
export default function MisLotes({ onVolver }) {
    ...
    const cargarLotes = useCallback(async () => {
        try {
            ...
            if (filtroActivo.tipo === 'fecha' && fechaIngreso.trim()) {
                params.append('fecha_ingreso', fechaIngreso.trim());
            }
            ...
            Alert.alert('Error de conexion', 'No se pudo conectar con el servidor para cargar lotes.');
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    ...
    const guardarEdicion = async () => {
        ...
        try {
            ...
            setLotes((prev) => prev.map((lote) => (
                lote.id_lote === actualizado.id_lote ? actualizado : lote
            )));
            setLoteSeleccionado(actualizado);
            setEditando(false);
            Alert.alert('Lote actualizado', 'Los cambios se guardaron correctamente.');
        } catch (error) {
            Alert.alert('Error de conexion', 'No se pudo conectar con el servidor para actualizar el lote.');
        } finally {
            setGuardando(false);
        }
    };
    ...
    const confirmarEliminar = (lote) => {
        Alert.alert(
            'Eliminar lote',
            `Se eliminara el lote ID ${lote.id_lote}. Esta accion no se puede deshacer.`,
            [
    ...
    const eliminarLote = async (lote) => {
        ...
        try {
            ...
            Alert.alert('Error de conexion', 'No se pudo conectar con el servidor para eliminar el lote.');
        } finally {
            setEliminandoId(null);
        }
    };

    const CampoEdicion = ({ grupo, campo, label, keyboardType = 'default' }) => (
    ...
    const OpcionesEdicion = ({ grupo, campo, label, opciones }) => (
        <View style={styles.campo}>
            <Text style={styles.label}>{label}</Text>
            <View style={styles.opcionesFila}>
                {opciones.map((opcion) => {
                    const activo = formEdicion[grupo][campo] === opcion;
                    return (
                        <TouchableOpacity
                            key={opcion}
                            style={[styles.chipOpcion, activo && styles.chipOpcionActivo]}
                            onPress={() => actualizarForm(grupo, campo, opcion)}
                        >
                            <Text style={[styles.chipOpcionTexto, activo && styles.chipOpcionTextoActivo]}>{opcion}</Text>
                        </TouchableOpacity>
                    );
                })}
            </View>
    ...
    const ToggleArete = () => (
        <TouchableOpacity
            ...
            onPress={() => actualizarForm('animal', 'arete_faltante', !formEdicion.animal.arete_faltante)}
        >
        ...
        >
            ...
            <View style={styles.encabezadoFila}>
                <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.subtitulo}>Selecciona un lote para ver detalles, editarlo o eliminarlo.</Text>
                </View>
                    ...
                    <View style={styles.dropdownLista}>
                        {FILTROS_RAPIDOS.map((opcion) => (
                            <TouchableOpacity
                                ...
                                onPress={() => seleccionarFiltro(opcion)}
                            >
                                <Text style={[
                                    styles.dropdownItemTexto,
                                    filtroActivo.value === opcion.value && styles.dropdownItemTextoActivo
                                ]}>
                                    {opcion.label}
                                </Text>
                                {filtroActivo.value === opcion.value && (
                                    <Ionicons name="checkmark" size={17} color="#002855" />
                                )}
                            </TouchableOpacity>
                        ))}
                    </View>
            ...
            {loading ? (
                ...
                lotes.map((lote) => (
                    <TouchableOpacity
                        ...
                        style={styles.filaLote}
                        onPress={() => abrirDetalle(lote)}
                        activeOpacity={0.85}
                    >
                        <View style={styles.filaIcono}>
                            <Ionicons name="cube" size={18} color="#002855" />
                        </View>
                        <Text style={styles.filaTexto}>ID lote: {lote.id_lote}</Text>
                        {eliminandoId === lote.id_lote ? (
                            <ActivityIndicator color="#002855" />
                        ) : (
                ...
                {loteSeleccionado && (
                    <ScrollView style={styles.modalPantalla} contentContainerStyle={styles.modalContenido}>
                        <View style={styles.modalHeader}>
                            <View>
                                <Text style={styles.modalTitulo}>Lote ID {loteSeleccionado.id_lote}</Text>
                                <Text style={styles.modalSubtitulo}>{loteSeleccionado.codigo_lote}</Text>
                            </View>
                        ...
                        <View style={styles.detalleCard}>
                            <View style={styles.detalleHeader}>
                                <Text style={styles.detalleTitulo}>Informacion completa</Text>
                            ...
                            <Text style={styles.detalle}><Text style={styles.bold}>Ingreso:</Text> {loteSeleccionado.fecha_ingreso}</Text>
                            <Text style={styles.detalle}><Text style={styles.bold}>Vencimiento:</Text> {loteSeleccionado.fecha_vencimiento}</Text>
                            <Text style={styles.detalle}><Text style={styles.bold}>Arete:</Text> {loteSeleccionado.num_arete || 'N/D'}</Text>
                            <Text style={styles.detalle}><Text style={styles.bold}>Sexo:</Text> {loteSeleccionado.sexo || 'N/D'}</Text>
                            <Text style={styles.detalle}><Text style={styles.bold}>Clasificacion:</Text> {loteSeleccionado.clasificacion || 'N/D'}</Text>
                            <Text style={styles.detalle}><Text style={styles.bold}>Edad:</Text> {loteSeleccionado.meses_edad ?? 'N/D'} meses</Text>
                            <Text style={styles.detalle}><Text style={styles.bold}>Guia:</Text> {loteSeleccionado.folio_guia || 'N/D'}</Text>
                        </View>
            ...
            <Modal visible={editando} animationType="slide" onRequestClose={() => setEditando(false)}>
                <ScrollView style={styles.modalPantalla} contentContainerStyle={styles.modalContenido}>
                    ...
                    <View style={styles.detalleCard}>
                        ...
                        <CampoEdicion grupo="lote" campo="codigo_lote" label="Codigo de lote" />
                        <CampoEdicion grupo="lote" campo="tipo_corte" label="Tipo de corte" />
                        <CampoEdicion grupo="lote" campo="peso_kg" label="Peso kg" keyboardType="decimal-pad" />
                        <CampoEdicion grupo="lote" campo="fecha_ingreso" label="Fecha de ingreso" />
                        <CampoEdicion grupo="lote" campo="fecha_vencimiento" label="Fecha de vencimiento" />
                        <OpcionesEdicion grupo="lote" campo="estado" label="Estado" opciones={ESTADOS_LOTE} />
                    </View>
                    ...
                    <View style={styles.detalleCard}>
                        ...
                        <View style={styles.campo}>
                            ...
                            <View style={styles.especieBloqueada}>
                                ...
                                <Text style={styles.especieTexto}>
                                    {nombreEspecie(loteSeleccionado?.especie)} ({loteSeleccionado?.especie})
                                </Text>
                            </View>
                        </View>
                        <CampoEdicion grupo="animal" campo="num_arete" label="Numero de arete" />
                        <OpcionesEdicion grupo="animal" campo="sexo" label="Sexo" opciones={SEXOS} />
                        <OpcionesEdicion grupo="animal" campo="clasificacion" label="Clasificacion" opciones={CLASIFICACIONES} />
                        <CampoEdicion grupo="animal" campo="meses_edad" label="Edad en meses" keyboardType="numeric" />
                        <ToggleArete />
                    </View>
...
const styles = StyleSheet.create({
    ...
    filaLote: { minHeight: 58, borderWidth: 1, borderColor: '#e2e8f0', borderRadius: 10, paddingHorizontal: 12, marginBottom: 10, flexDirection: 'row', alignItems: 'center', backgroundColor: '#ffffff' },
    filaIcono: { width: 34, height: 34, borderRadius: 8, backgroundColor: '#eff6ff', alignItems: 'center', justifyContent: 'center', marginRight: 10 },
    filaTexto: { flex: 1, color: '#0f172a', fontSize: 16, fontWeight: 'bold' },
    ...
    badgeEstado: { borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6 },
